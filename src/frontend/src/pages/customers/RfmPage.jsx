...
function ActionExecutionDrawer({ config, onClose, onSuccess }) {
  ...
  useEffect(() => {
    ...
    if (config.action === 'winback') {
      setMessage(`Chào anh/chị, để tri ân khách hàng cũ, cửa hàng gửi tặng anh/chị mã voucher ${discountText} áp dụng cho mọi đơn hàng tại Shopee. Chúc anh/chị có trải nghiệm mua sắm tuyệt vời!`)
    }
    ...
    setTimeout(() => {
      ...
      try {
        ...
        list.unshift({
          ...
          action: 'winback',
          target: `${config.segment} (${config.count} KH)`,
          method: method,
      ...
      onSuccess(`Đã kích hoạt thành công chiến dịch kích hoạt lại cho phân khúc ${config.segment}!`)
      onClose()
    }, 1500)
  }

  const title = 'Kích hoạt lại khách hàng cũ'
  const subtitle = `Phân khúc: ${config.segment} (${config.count} khách hàng)`
...
export default function RfmPage() {
  ...
  const handleCreateCampaign = (segment, count) => {
    setExecutionDrawer({ action: 'winback', segment, count })
  }
